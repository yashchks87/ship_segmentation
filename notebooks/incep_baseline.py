...
K.clear_session()
log_dir = f"{os.environ['tb_path']}classification/incepv3_baseline/"
if os.path.exists(log_dir) == False:
    os.makedirs(log_dir)
tensorboard_callback = tf.keras.callbacks.TensorBoard(log_dir = log_dir)
weights_path = '/home/ubuntu/ml-data-training/weights/weights/incepv3_baseline/'
weights_save = CallbackForSavingModelWeights(weights_path)
batch_size = 512
train_dataset = get_data(train)
val_dataset = get_data(val, repeat = False, shuffle = False)
model = create_model('InceptionV3', (256, 256, 3))
